...
@section('content')
    <section class="credential_form signForm">
        <form method="POST" name="change_pass_frm" id="change_pass_frm" action="{{ route('reset.password') }}">
            @csrf
            <input type="hidden" name="userCode" value="{{ session('USER_DETAILS.USER_CODE') ?? null }}">
            ...
            <div class="login_page main_pg inner-cred">
                ...
                <center>
                    ...
                    @else
                        <p style="color:red; font-weight: 600;">You have a temporary password. You must change your password
                            before proceeding.</p>
                        <p style="color:red">
                            @isset($data['app']['msg'])
                                {{ $data['app']['msg'] }}
