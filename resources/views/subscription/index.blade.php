    ...
    <div class="responsive-container-block big-container">
        <div class="responsive-container-block container">
            <div class="responsive-container-block card-container">
                ...
                $descItems = explode("\n", strip_tags($plan->plan_desc)); // Assuming description is newline-separated points
                @endphp
                ...
                <div class="responsive-cell-block wk-desk-4 wk-ipadp-4 wk-tab-6 wk-mobile-12">
                    <div class="card">
                        ...
                        <h1 class="monthly-price {{ $plan->plan_period == 'month' ? '' : 'd-nones' }}">
                            ${{ $plan->plan_amount }}
                        </h1>
                        <h1 class="yearly-price {{ $plan->plan_period == 'year' ? '' : 'd-nones' }}">
                            ${{ $plan->plan_amount }}
                        </h1>
                        <span class="month">{{ $periodStr }}</span>
                        <div class="card-description">
                            <span class="monthly-plan {{ $plan->plan_period == 'month' ? '' : 'd-nones' }}">
                                @foreach ($descItems as $item)
                                <p class="text-blk card-points">{{ $item }}</p>
                                @endforeach
                            </span>
                            <span class="yearly-plan {{ $plan->plan_period == 'year' ? '' : 'd-nones' }}">
                                @foreach ($descItems as $item)
                                <p class="text-blk card-points">{{ $item }}</p>
                                @endforeach
                            </span>
...
<script>
    $(document).ready(function() {
        // Toggle pricing view between Monthly and Yearly
        $('input[type="checkbox"]').change(function() {
            if ($(this).is(":checked")) {
                $('.monthly-price, .monthly-plan').addClass("d-nones");
                $('.yearly-price, .yearly-plan').removeClass("d-nones");
            } else {
                $('.monthly-price, .monthly-plan').removeClass("d-nones");
                $('.yearly-price, .yearly-plan').addClass("d-nones");
            }
        });
